...
**Follow-up migrations** — status:
- R5: ~~5 HTTP + 14 process-spawn sites~~ — landed in commits `3215188` (4/5 HTTP migrated; 2 stayed bespoke for header-walking auth flow + service clients) and `54d9b5f` (6/14 proc-spawn migrated; 8 stayed bespoke — long-lived detached children, fire-and-forget stdio-ignored reapers, type decls).
- R10: ~~15 main.zig settings~~ — landed in commit `21e7366`. 8 migrated to `util_setting.{loadBool,loadInt,loadFloat,readSingleLine}`; one (`loadAutoCheck` — multi-line key=value parser) kept inline because its shape doesn't fit a single-line helper. Net delete −93 LOC in main.zig.
- R11: ~~existing tests on bespoke fixtures~~ — landed in commit `6222ce7`. 41 tests across 6 modules migrated to `util/test_env.TestEnv`; 11 left bespoke (installer/apply, downloads/archive — they centralize through a tar/zip-build helper that R5 separately migrated to `util_proc.run`, so they're already DRY).
- R6 (ImageJob / PostInstallJob / ManualInstallJob): ~~bespoke jobs~~ — landed in commits `7fa4ad0` (ImageJob via cancel-pointer-on-payload) and `761c47a` (PostInstall + ManualInstall by accepting the paired poller thread as worker-local). All 10 worker types now use `Job(Payload)`; the file-local phase enums `PostInstallPhase` / `ManualInstallPhase` / `ImageJobPhase` retired.
- R3: remaining `_buf`+`_len` pairs (err_msg in WizardBlock, aria2_port_msg / aria2_seed_ratio_msg with inline memcpy, modfile_id, for_game) — still outstanding. Lowest-value of the follow-ups.
